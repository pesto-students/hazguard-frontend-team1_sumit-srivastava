...
const SinglePost = ({ data, fromMyPosts }) => {
	...
	return (
		<div className="w-[100%] p-3 sxl:h-[180px] md:h-[170px] min-h-[250px] max-h-[200px] first:mt-[0px] mt-[10px] bg-[#fff] rounded-[20px] flex flex-col items-center justify-center text-ellipsis mb-3">
			...
			<div className="w-[100%] h-[45%] max-h-[50px] px-3 flex sxl:flex-col md:flex-row justify-between sxl:items-between md:items-center">
				...
				<div className="sxl:w-full md:w-2/5 h-[60%] flex sxl:justify-end md:justify-end items-center mt-1">
					...
					{data.userId === userData.userId && fromMyPosts && (
						<>
					...
					<div className={`flex items-center ${!fromMyPosts && "mr-4"}`}>
					...
					{!fromMyPosts ? (
						...
						<button
							onClick={() => navigate(`/post/${data.hazardId}`)}
							className="bg-[#EED132] ml-2 flex justify-center items-center text-[#000] sxl:w-[150px] md:w-[200px] sxl:h-[28px] md:h-[45px] rounded-[40px] sxl:text-[13px] md:text-[18px] font-[700] hover:bg-[rgba(238,209,50,0.8)]"
						>
							VIEW MORE
						</button>
					)}
