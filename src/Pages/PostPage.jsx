...
import { deleteHazard } from "../Helpers/hazard";
import { setCheckChange, setLoading, addToSavedPost, removeFromSavedPost } from "../Store/storingData";
...
const PostPage = () => {
	...
	return (
		<>
			...
			) : userData.subscriptionType === "International" || (userData.subscriptionType === "National" && userData.country === hazard.country) ? (
					...
					<div className="w-[100%] h-[100%] bg-[#fff] rounded-[20px] sxl:px-[15px] md:px-[30px] py-[30px] overflow-y-hidden">
						...
						<div className="w-full h-[2%] flex justify-end items-start sxl:flex-wrap md:flex-nowrap">
							...
							<div className="w-[100%] flex sxl:justify-center md:justify-end items-center sxl:mt-3 md:mt-0">
								<p className="text-white w-fit bg-red-400 md:inline-block sxl:text-[12px] sm:text-[14px] sxl:mr-1 md:mr-2 md:text-[15px] px-2 py-1 font-normal rounded-sm">
									{hazard.industry}
								</p>
								<p className="text-white w-fit bg-yellow-400 md:inline-block sxl:text-[12px] sm:text-[14px] sxl:mr-1 md:mr-2 md:text-[15px] px-2 py-1 font-normal rounded-sm">
									{hazard.type}
								</p>
